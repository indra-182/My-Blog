...
import type { Locale, PostDocument, PostFrontmatter, PostSummary } from "./post-types";
...
function defaultRootDirectory() {
  return path.resolve(
    /* turbopackIgnore: true */ process.env.CONTENT_ROOT ?? path.join(process.cwd(), "content/posts"),
  );
}

function isPublished(post: PostSummary, includeDrafts: boolean) {
  return includeDrafts || (!post.draft && new Date(post.publishedAt).getTime() <= Date.now());
}

export function parsePostSource(source: string, filePath: string, expectedLocale?: Locale): PostFileResult {
  try {
    ...
    if (expectedLocale && result.data.locale !== expectedLocale) {
      return {
        success: false,
        filePath,
        error: new Error(`locale does not match directory: expected ${expectedLocale}`),
      };
    }
    if (!result.data.draft && new Date(result.data.publishedAt).getTime() > Date.now()) {
      return {
        success: false,
        filePath,
        error: new Error("publishedAt cannot be in the future unless draft is true"),
      };
    }
...
async function readLocalePosts(rootDirectory: string, locale: Locale): Promise<PostFileResult[]> {
  const localeDirectory = path.join(rootDirectory, locale);
  let filenames: string[];
  try {
    filenames = (await readdir(localeDirectory)).filter((filename) => filename.endsWith(".mdx"));
  } catch (error) {
  ...
  return Promise.all(
    filenames.map(async (filename) => {
      ...
      return parsePostSource(await readFile(filePath, "utf8"), filePath, locale);
    }),
  );
}
...
export function createPostRepository(rootDirectory = defaultRootDirectory()) {
  async function getAllPosts(locale: Locale, options?: { includeDrafts?: boolean }) {
    const includeDrafts = options?.includeDrafts ?? false;
    const parsed = await readLocalePosts(rootDirectory, locale);
    const posts = parsed
      .filter((item): item is Extract<PostFileResult, { success: true }> => item.success)
      .map((item) => item.data)
  ...
  async function getRelatedPosts(post: PostSummary, limit = 3) {
    ...
    return posts
      ...
      .map((candidate) => ({
        candidate,
        score: candidate.topics.filter((topic) => topicSet.has(topic.toLowerCase())).length,
      }))
  ...
  async function getSeriesNeighbors(post: PostSummary) {
    if (!post.series || post.seriesOrder === undefined) return { previous: null, next: null };
    const posts = (await getAllPosts(post.locale))
  ...
  async function getTranslationPath(translationKey: string, targetLocale: Locale) {
    const posts = await getAllPosts(targetLocale);
    const post = posts.find((candidate) => candidate.translationKey === translationKey);
    return post ? `/${targetLocale}/blog/${post.slug}` : null;
  }

  return { getAllPosts, getPostBySlug, getRelatedPosts, getSeriesNeighbors, getTranslationPath };
}
