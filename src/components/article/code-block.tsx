...
export function CodeBlock({ code, highlightedCode, language, dictionary }: { code: string; highlightedCode?: ReactNode; language?: string; dictionary: Dictionary }) {
  ...
  const label = status === "copied" ? dictionary.article.copied : dictionary.article.copyCode;
  return (
    <div className="code-block">
      ...
      <div className="sr-only" aria-live="polite">{status === "failed" ? dictionary.article.copyFailed : status === "copied" ? dictionary.article.copied : ""}</div>
    </div>
  );
}
