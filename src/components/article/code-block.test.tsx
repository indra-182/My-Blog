...
describe("CodeBlock", () => {
  ...
  it("keeps syntax-highlighted markup while copying plain code", async () => {
    ...
    const { container } = render(
      <CodeBlock
        code="const value = 1"
        highlightedCode={<span style={{ color: "#ff79c6" }}>const value = 1</span>}
        language="ts"
    ...
    expect(codeBlock.getByText("const value = 1", { selector: "span" })).toHaveStyle({ color: "rgb(255, 121, 198)" });
    await act(async () => { fireEvent.click(codeBlock.getByRole("button", { name: "Copy code" })); });
    expect(writeText).toHaveBeenCalledWith("const value = 1");
  });
});
