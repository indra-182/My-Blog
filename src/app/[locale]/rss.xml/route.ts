...
export async function GET(
  ...
  const items = posts
    .map(
      (post) =>
        `<item><title>${escapeXml(post.title)}</title><link>${siteConfig.blogUrl}/${locale}/blog/${post.slug}</link><guid>${siteConfig.blogUrl}/${locale}/blog/${post.slug}</guid><description>${escapeXml(post.description)}</description><pubDate>${new Date(post.publishedAt).toUTCString()}</pubDate><category>${post.topics.map(escapeXml).join("</category><category>")}</category></item>`,
    )
    .join("");
