...
export async function GET() {
  ...
  const items = posts
    ...
    .join("");
  const xml = `<?xml version="1.0" encoding="UTF-8"?><rss version="2.0"><channel><title>INDRA.DEV — Catatan engineering</title><link>${siteConfig.blogUrl}</link><description>Catatan teknis praktis dari INDRA.DEV</description>${items}</channel></rss>`;
