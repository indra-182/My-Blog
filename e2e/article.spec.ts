...
test("article exposes reading primitives and copy code", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "Separating Server State from UI State" })).toBeVisible();
  await expect(page.getByRole("navigation", { name: "Breadcrumb" })).toBeVisible();
  await expect(page.getByRole("link", { name: /Keep the URL shareable/i })).toBeVisible();
